//Problem: https://codeforces.com/contest/1546/problem/A
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        if (a[i] > b[i]) {
            for (int j = 0; j < a[i]-b[i]; j++) down.pb(i+1);
        }
        if (b[i] > a[i]) {
            for (int j = 0; j < b[i]-a[i]; j++) up.pb(i+1);
        }
    }
    if (up.size() != down.size()) cout << "-1\n";
    else {
        cout << down.size() << '\n';
        for (int i = 0; i < down.size(); i++) cout << down[i] << " " << up[i] << '\n';
    }
