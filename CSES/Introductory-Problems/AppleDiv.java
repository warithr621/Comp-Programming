...
public class AppleDiv {
    ...
    static void subset(List<List<Long>> sub, ArrayList<Long> p, ArrayList<Long> output, int idx) {
        if (idx == p.size()) {
            sub.add(output);
            return;
        }
        subset(sub, p, new ArrayList<>(output), idx+1);
        output.add(p.get(idx));
        subset(sub, p, new ArrayList<>(output), idx+1);
    }
	...
	public static void solve() throws IOException {
	    ...
	    List<List<Long>> sub = new ArrayList<>();
        subset(sub, p, new ArrayList<>(), 0);
        ...
        for (int i = 0; i < sub.size(); i++) {
            ...
            diff = Math.min(diff, Math.abs(2 * cur - sum));
        }
        out.println(diff);
	}
}
